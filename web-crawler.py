...
class WebCrawler:
  ...
  def parse_page(self,html_document,depth,query): #Extract all the links from the page and add them to the URLs list
    ...
    for link in soup.findAll('a', attrs={'href': re.compile("^(http|https)://")}):
      if normalize_url(link) not in self.visited:
        href = link.get('href')
  ...
  def crawl(self):
    ...
    while self.depth_reached <= 5: #initially trying till depth 5
      url_tuple = urls.pop(0)
      url = url_tuple[0]
      depth = url_tuple[1]
      ...
      self.visited[normalize_url(url)] = depth
      if mime_type in self.valid_mime_types
        parse_page(document,depth,self.query)
